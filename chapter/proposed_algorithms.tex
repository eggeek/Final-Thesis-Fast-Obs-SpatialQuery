...
heuristic function for guidance, nodes can only be prioritised by the \textit{g-value} of their
...
\begin{itemize}
  \item The Interval Heuristic, which prioritises nodes using the closest point from its
    associated interval.
...
\begin{figure}[htp]
  ...
  $t$ is a target, green dashed segments are interval of expanded search nodes.
  From the figure we can see that the algorithm does unnecessary expansion in the direction that no target.}
  \label{hv}
\end{figure}
\noindent
...
instead of flipping targets, we filp $r$, and thus formed six areas as shown in Figure~\ref{fa}.
...
for same reason, we can combine $areaB, areaB'$, so finally we formed four areas.
...
\section{Summary}
In this chapter, we start with two straightforward adaption of point-to-point Polyany
and finally proposed two heuristics for the Polyanya OkNN algorithm.
For \textit{Interval Heuristic}, it support multi-target by
removing $t$ from \textit{h-value}; for \textit{Target Heuristic}, it support multi-target by
